...
from . import (send, order, btcpay, issuance, broadcast, bet, dividend, burn, cancel, rps, rpsresolve, publish)
...
def db_query(db, statement, bindings=(), callback=None, **callback_args):
    ...
    for word in forbidden_words:
        if word in statement.lower() or any([word in str(binding).lower() for binding in bindings]):
            raise exceptions.SanityError("Forbidden word in query: '{}'.".format(word))
...
class APIStatusPoller(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            try:
                if not config.FORCE and (time.time() - self.last_check > 10 * 60): # Ten minutes since last check.
                    code = 11
...
class APIServer(threading.Thread):
    ...
    def run(self):
        ...
        def get_unspent_txouts(address):
            return bitcoin.get_unspent_txouts(address, unconfirmed=False)
        ...
        if not config.UNITTEST:  #skip setting up logs when for the test suite
            api_logger = (logging.getLogger('werkzeug'), app.logger)
