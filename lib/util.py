...
def price (numerator, denominator, block_index):
    if block_index >= 0 or config.TESTNET:
...
def version_check (db):
    if config.FORCE:
        return
    try:
        host = 'https://raw.githubusercontent.com/energyparty/energypartyd/master/version.json'
    ...
    if not passed:
        explanation = 'Your version of energypartyd is v{}, but, as of block {}, the minimum version is v{}.{}.{}. Reason: ‘{}’. Please upgrade to the latest version and restart the server.'.format(config.VERSION_STRING, versions['block_index'], versions['minimum_version_major'], versions['minimum_version_minor'], versions['minimum_version_revision'], versions['reason'])
...
def database_check (db, blockcount):
    """Checks {} database to see if the {} server has caught up with Energycoind.""".format(config.XCP_NAME, config.XCP_CLIENT)
    if last_block(db)['block_index'] + 1 < blockcount:
        raise exceptions.DatabaseError('{} database is behind Energycoind. Is the {} server running?'.format(config.XCP_NAME, config.XCP_CLIENT))
    return
...
def debit (db, block_index, address, asset, quantity, action=None, event=None):
    ...
    if asset == config.BTC:
        raise exceptions.BalanceError('Cannot debit EnergyCoins from a {} address!'.format(config.XCP_NAME))
    ...
    if divisible:
        if dest == 'output':
            ...
            if quantity == quantity.to_integral():
                ...
                raise exceptions.QuantityError('Divisible assets have only six decimal places of precision.')
        else:
            return quantity
    else:
        quantity = D(quantity)
...
def xcp_supply (db):
    ...
    cursor.execute('''SELECT SUM(earned) AS total FROM burns \
                      WHERE status = ?''', ('valid',))
    burn_total = list(cursor)[0]['total'] or 0
