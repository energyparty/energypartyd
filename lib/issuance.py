...
def validate (db, source, destination, asset, quantity, divisible, callable_, call_date, call_price, description, block_index):
    ...
    if issuances:
        ...
        if last_issuance['call_date'] > call_date and (call_date != 0 or (block_index < 0 and not config.TESTNET)):
    ...
    if quantity or config.TESTNET:
        if not reissuance or config.TESTNET:
            cursor = db.cursor()
