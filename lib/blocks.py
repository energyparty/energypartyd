...
from . import (send, order, btcpay, issuance, broadcast, bet, dividend, burn, cancel, rps, rpsresolve)
...
TABLES = ['credits', 'debits', 'messages'] + \
         ['bet_match_resolutions', 'order_match_expirations',
          ...
          'burns', 'cancels', 'dividends', 'issuances', 'sends',
...
def follow (db):
    ...
    try:
        ...
        if user_version == config.VERSION_MINOR:
           minor_version = user_version
           major_version = config.VERSION_MAJOR
           user_version = (config.VERSION_MAJOR * 1000) + minor_version
           cursor.execute('PRAGMA user_version = {}'.format(user_version))
        else:
           minor_version = user_version % 1000
           major_version = user_version // 1000
        if major_version != config.VERSION_MAJOR:
            logging.info('Status: client major version number mismatch ({} ≠ {}).'.format(major_version, config.VERSION_MAJOR))
            reparse(db, block_index=config.BLOCK_FIRST, quiet=False)
            logging.info('Status: update database version')
            user_version = (config.VERSION_MAJOR * 1000) + config.VERSION_MINOR
            cursor.execute('PRAGMA user_version = {}'.format(user_version))
        elif minor_version != config.VERSION_MINOR:
            logging.info('Status: client minor version number mismatch ({} ≠ {}).'.format(minor_version, config.VERSION_MINOR))
            reparse(db, block_index, quiet=False)
    ...
    while True:
        ...
        if block_index <= block_count:
            ...
            while True:
                if current_index == config.BLOCK_FIRST: break

                c_hash = bitcoin.get_block_hash(current_index)
            ...
            if requires_rollback:
                ...
                logging.warning('Status: Blockchain reorganisation at block {}.'.format(current_index))
                util.message(db, block_index, 'reorg', None, {'block_index': current_index})
            ...
            block_hash = bitcoin.get_block_hash(current_index)
