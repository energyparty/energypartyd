...
def format_bet (bet):
    ...
    return [util.BET_TYPE_NAME[bet['bet_type']], bet['feed_address'], util.isodt(bet['deadline']), target_value, leverage, str(bet['wager_remaining'] / config.UNIT) + ' XEP', util.devise(db, odds, 'odds', 'output'), bet['expire_index'] - util.last_block(db)['block_index'], bet['tx_hash']]
...
def set_options (data_dir=None, backend_rpc_connect=None,
    ...
    if not unittest:
        if config.TESTCOIN:
            ...
            config.PREFIX = b'ENRGPRTY'             # 8 bytes
    else:
    ...
    if database_file:
        ...
        string = '{}.'.format(config.XCP_CLIENT)
        if config.TESTNET:
            string += '.testnet'
        if config.TESTCOIN:
            string += '.testcoin'
...
def balances (address):
    ...
    btc_balance = sum(out['amount'] for out in bitcoin.get_unspent_txouts(address, unconfirmed=False))
    table.add_row([config.BTC, btc_balance])
...
if __name__ == '__main__':
    ...
    parser_bet.add_argument('--wager', required=True, help='the quantity of XEP to wager')
    parser_bet.add_argument('--counterwager', required=True, help='the minimum quantity of XEP to be wagered by the user to bet against you, if he were to accept the whole thing')
    ...
    parser_dividend.add_argument('--quantity-per-unit', required=True, help='the quantity of XEP to be paid per whole unit held of ASSET')
    ...
    parser_burn = subparsers.add_parser('burn', help='destroy {} to earn XEP, during an initial period of time')
    ...
    parser_rps.add_argument('--wager', required=True, help='the quantity of XEP to wager')
    ...
    parser_rps.add_argument('--fee', help='the exact ENRG fee to be paid to miners')
    ...
    parser_rpsresolve.add_argument('--fee', help='the exact ENRG fee to be paid to miners')
    ...
    if args.action == 'send':
        ...
        if args.fee: args.fee = util.devise(db, args.fee, 'ENRG', 'input')
    ...
    # VIEWING (temporary)
        ...
        call_price = str(results['call_price']) + ' XEP' if results['call_price'] else results['call_price']
        ...
        for asset in totals.keys():
            ...
            total_table.add_row([asset, round(balance, 6)])
        print('TOTAL')
